...
def generator(data,n,vocab_size, sequence_length,x_text,ans_labels,num_labels,batch_size):
    
    path = 'CLEVR_v1.0'
    questions_path = path + '/questions/CLEVR_' + 'train' + '_questions.json'
    images_path = path + '/'+ 'images/train' + '/'
     
    a=list(zip(x_text,ans_labels,data))
    random.shuffle(a)		  
    x_text[:],ans_labels[:],data[:] =zip(*a) 
    x_text=x_text[0:n]
    ...
    while True:
        ...
        z = list(zip(x_text,ans_labels,[x['image_filename'] for x in data]))
        random.shuffle(z)
